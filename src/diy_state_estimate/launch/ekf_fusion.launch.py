#!/usr/bin/env python3
"""
ekf_fusion.launch.py — wheel + IMU + FAST-LIO2 → /odom and odom→base_link TF
═══════════════════════════════════════════════════════════════════════════
Starts two nodes:
...
PRECONDITIONS
  * Nothing else may publish the odom→base_link TF. FAST-LIO2 does so
    unconditionally, so master.launch.py remaps its /tf away.
  ...
  * The URDF must provide imu_link → base_link (robot_description).
"""
...
def generate_launch_description():
    ...
    relay_node = Node(
        ...
        parameters=[{
            ...
            # lidar_pose_cov_floor stays at the node default [0.04, 0.04, 0.02];
    ...
    return LaunchDescription(declare_args + [relay_node, ekf_node])
