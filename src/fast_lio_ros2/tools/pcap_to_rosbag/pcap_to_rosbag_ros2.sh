...
# Convert a Hesai QT64 PCAP file to a FAST-LIO2-ready rosbag2 (ROS 2).
#
# NOTE: QT64 has no built-in IMU. The Hesai ROS 2 Driver's
# ros_send_imu_topic will NOT contain usable IMU data when replaying a
# bare QT64 PCAP -- you must record your external IMU (e.g. ACEINNA)
# separately and merge/replay it alongside this bag for FAST-LIO2.
#
# Pipeline:
#   input.pcap
...
# ── wait for topics ───────────────────────────────────────────────────────────
# Only the point cloud topic is required: QT64 has no built-in IMU, so the
# driver's IMU topic will not carry real data when replaying a bare PCAP.
info "Waiting for point cloud topic (timeout: ${TOPIC_WAIT_TIMEOUT}s)..."
...
while true; do
    ...
    "$pc_ok" && break
    [[ $(date +%s) -lt $deadline ]] || die "Topic $LIDAR_TOPIC did not appear within ${TOPIC_WAIT_TIMEOUT}s."
    sleep 1
done
ok "Topic found: $LIDAR_TOPIC"
...
# Only the point cloud is recorded -- see the IMU note above.
OUTPUT_PARENT=$(dirname "$OUTPUT")
[[ -d "$OUTPUT_PARENT" ]] || mkdir -p "$OUTPUT_PARENT"
info "Recording rosbag2 → $OUTPUT"
ros2 bag record -o "$OUTPUT" "$LIDAR_TOPIC" &
BAG_PID=$!
sleep 1
...
echo ""
echo "  Terminal 1:"
echo "    cd ~/localizer && source install/setup.bash"
echo "    ros2 launch fast_lio_ros2 lio_localizer.launch.py"
echo ""
echo "  Terminal 2:"
echo "    ros2 bag play $OUTPUT"
echo ""
echo -e "${YEL}Notes:${NC}"
echo "  - QT64 has no built-in IMU: this bag only contains $LIDAR_TOPIC."
echo "    Record/merge your external IMU (e.g. ACEINNA) data separately"
echo "    before running FAST-LIO2."
echo "  - Verify imu_gyr_unit in config/qt64.yaml matches your IMU driver's output (deg or rad)."
...
echo "      ros2 bag play $OUTPUT &"
echo "      ros2 run fast_lio_ros2 check_input.py"
echo ""
