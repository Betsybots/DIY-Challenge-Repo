...
"""
check_config.py  —  Statically validate a FAST-LIO2 (Hesai QT64) yaml config

Does NOT require ROS to be running. Parses the yaml and checks parameter
consistency for the Hesai QT64 LiDAR (ROS 2), catching the most common
misconfigurations before launch.

Checks:
  - preprocess.lidar_type matches QT64
  - preprocess.scan_line matches QT64
...
# expected values for the Hesai QT64 LiDAR (ROS 2)
QT64_SPEC = {"scan_line": 64, "lidar_type": 3}
...
class ConfigChecker:
    ...
    def _check_lidar_type(self, cfg):
        ...
        if val is None:
            ...
            self._pass(f"preprocess.lidar_type={val} matches QT64")
    ...
    def _check_scan_line(self, cfg):
        ...
        if val is None:
            ...
            self._pass(f"preprocess.scan_line={val} matches QT64")
...
def main():
    parser = argparse.ArgumentParser(
        description="Statically validate a FAST-LIO2 (Hesai QT64) yaml config")
    ...
    print(f"\n{'='*62}")
    print(f"  FAST-LIO2 Config Checker  |  QT64  |  ROS 2")
