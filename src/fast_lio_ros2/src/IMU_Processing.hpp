...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    last_max_imu_gap_ = std::max(last_max_imu_gap_, tail_stamp - head_stamp);
    ...
    // Guard against non-monotonic IMU timestamps (out-of-order messages or
    // a clock jump). This is more likely with an external IMU running its
    // own driver/clock -- as used alongside QT64, which has no built-in
    // IMU -- than with an IMU integrated into the LiDAR's own clock domain.
    // Feeding a non-positive dt into kf_state.predict() corrupts the filter
    // state, which can look like the pose/map swinging wildly even though
    ...
    if (dt <= 0.0)
    {
      ...
      if (warn_count++ % 100 == 0)
      {
        printf("[ImuProcess] WARNING: skipping non-monotonic IMU sample "
               "(dt=%.6f s, head=%.6f, tail=%.6f). Check IMU driver "
               "timestamps / clock synchronization.\n", dt, head_stamp, tail_stamp);
      }
      continue;
    }
    ...
    if (shock_this_step)
    {
      Eigen::Matrix<double, 12, 12> Q_shock = Q;
      Q_shock.block<3, 3>(0, 0) *= shock_process_noise_scale_;
      Q_shock.block<3, 3>(3, 3) *= shock_process_noise_scale_;
      kf_state.predict(dt, Q_shock, in);
    }
    else
    {
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      // Sanity-check the estimated gyro bias (mean_gyr, averaged while the
      // platform was assumed stationary during init). A genuinely stationary
      // MEMS gyro typically biases well under ~0.2 rad/s (~11 deg/s). A much
      // larger value usually means the raw angular_velocity units don't
      // match the `common.imu_gyr_unit` setting (deg/s mistaken for rad/s,
      // or vice versa). Only the constant part gets absorbed into this bias
      // -- a real "wiggle" (a time-varying signal) still comes through
      // scaled wrong, which is why small real motion can look like huge
      // apparent motion.
      if (mean_gyr.norm() > 0.2)
      {
        printf("[ImuProcess] WARNING: estimated gyro bias magnitude is %.3f rad/s "
               "(%.1f deg/s), which is unusually large for a stationary IMU. "
               "Double-check the 'common.imu_gyr_unit' (deg/rad) setting against "
               "what your IMU driver actually publishes.\n",
               mean_gyr.norm(), mean_gyr.norm() * 180.0 / M_PI);
      }
