...
#include "slam_interfaces/srv/refine_map.hpp"
#include "slam_interfaces/srv/save_poses.hpp"
...
public:
    HBANode() : Node("hba_node")
    {
        ...
        m_refine_map_srv = this->create_service<slam_interfaces::srv::RefineMap>(
            "refine_map",
            std::bind(&HBANode::refineMapCB, this, std::placeholders::_1, std::placeholders::_2));
        m_save_poses_srv = this->create_service<slam_interfaces::srv::SavePoses>(
            "save_poses",
    ...
    void refineMapCB(const std::shared_ptr<slam_interfaces::srv::RefineMap::Request> request, std::shared_ptr<slam_interfaces::srv::RefineMap::Response> response)
    {
        ...
        while (std::getline(ifs, line))
        {
            ...
            m_pose_file_names.push_back(file_name);
        }
    ...
    void savePosesCB(const std::shared_ptr<slam_interfaces::srv::SavePoses::Request> request, std::shared_ptr<slam_interfaces::srv::SavePoses::Response> response)
    {
        std::lock_guard<std::mutex> lock(m_service_mutex);
        ...
        {
            response->success = false;
            response->message = "failed to open pose output file";
            return;
        }
        const auto &poses = m_hba->poses();
        if (m_pose_file_names.size() != poses.size())
        {
            response->success = false;
            response->message = "pose and patch filename counts do not match";
            return;
        }
        for (size_t i = 0; i < poses.size(); ++i)
        {
            const Pose &pose = poses[i];
            const Eigen::Quaterniond q(pose.r);
            ofs << m_pose_file_names[i] << " "
    ...
    void saveRefinedMap()
    {
        if (m_maps_path.empty() || m_hba->poses().empty())
            return;
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "FAILED TO SAVE REFINED MAP: %s", map_path.string().c_str());
            return;
        }
...
private:
    ...
    rclcpp::Service<slam_interfaces::srv::RefineMap>::SharedPtr m_refine_map_srv;
    rclcpp::Service<slam_interfaces::srv::SavePoses>::SharedPtr m_save_poses_srv;
    ...
    std::vector<std::string> m_pose_file_names;
