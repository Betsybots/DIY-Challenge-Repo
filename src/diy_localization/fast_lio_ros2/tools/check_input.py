...
"""
check_input.py  —  Validate LiDAR and IMU inputs before running FAST-LIO2 (Hesai QT64)

Checks:
  1. /lidar_points topic exists
  2. /imu/data topic exists
  ...
  5. 'ring' range matches QT64 (0–63)
...
Usage (ROS 2):
    ...
    ros2 run fast_lio_ros2 check_input.py --lidar_topic /lidar_points --imu_topic /imu/data
"""
...
def _read_field(pc_msg, field_name, fmt, size, max_points=500):
    ...
    n = point_count if max_points is None else min(point_count, max_points)
...
class InputChecker:
    ...
    def run_ros2(self):
        ...
        # Keep only the newest sample. Deep queues let the single-threaded
        # diagnostic executor drain stale IMU messages while LiDAR advances,
        # which looks like a large clock offset even when the sensors agree.
        node.create_subscription(PointCloud2, self.lidar_topic, self._pc_cb, 1)
        node.create_subscription(Imu,         self.imu_topic,   self._imu_cb, 1)
    ...
    def _check_timestamp(self):
        vals = _read_field(self.pc_msg, "timestamp", "<d", 8, max_points=None)
        if vals is None:
    ...
    def _check_ring_range(self):
        ...
        if max_ring >= scan_line:
            ...
            print(f"{WARN} ring max={max_ring}, expected up to {scan_line - 1} "
                  f"({self.model_label}) — sparse frame?")
        else:
    ...
    def _check_gyro_unit(self):
        ...
        for m in self.imu_msgs[:50]:
            ...
            mags.append(math.radians(magnitude) if self.gyro_unit == "deg" else magnitude)
        median = sorted(mags)[len(mags) // 2]

        if median > math.radians(50):
            print(f"{WARN} gyro median ‖ω‖ = {median:.4f} rad/s — unusually high. "
                  f"Verify motion and --gyro-unit")
        else:
            print(f"{PASS} gyro magnitude: {median:.4f} rad/s after "
                  f"{self.gyro_unit} conversion — normal range")
    ...
    def _check_lidar_imu_sync(self):
        ...
        signed_offset_ms = (nearest - pc_t) * 1e3
        offset_ms = abs(signed_offset_ms)
...
def main():
    ...
    parser.add_argument("--imu_topic",   default="/imu/data",
                        help="IMU topic           (default: /imu/data)")
    parser.add_argument("--model",       default="qt64",
                        choices=list(MODEL_SPECS.keys()),
                        help="LiDAR model (default: qt64)")
    ...
    parser.add_argument("--gyro-unit", choices=["rad", "deg"], default="rad",
                        help="Unit published by the IMU driver (default: rad)")
