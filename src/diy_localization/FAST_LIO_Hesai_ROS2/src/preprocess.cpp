...
// the Hesai QT64 LiDAR.  Original upstream structure is retained
// where applicable; original copyright notices are preserved in the
// accompanying LICENSE file.
//
// Modified by Hesai Technology, 2026-06:
//   - Retained only Hesai LiDAR handler; removed all non-Hesai LiDAR paths
//   - Added hesai_handler() for PointCloud2 parsing
//   - Added per-point ring + timestamp extraction and frame accumulation
//
// Further modified, 2026-07:
//   - Added QT64 case to the lidar_type switch in process()
//   - hesai_handler() now validates that the required PointCloud2 fields
//     are present before calling pcl::fromROSMsg(), so a mismatched
//     driver/firmware point format fails with a clear diagnostic instead
//     of an out-of-bounds PCL read

#include "preprocess.h"

#include <pcl/common/common.h>

Preprocess::Preprocess() : feature_enabled(0), lidar_type(QT64), blind(0.01), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS = 16;
  SCAN_RATE = 10;
...
void Preprocess::hesai_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  // Validate required fields before conversion so a mismatched driver point
  // format (wrong firmware/SDK version, wrong LiDAR model selected, etc.)
  // produces a clear diagnostic instead of an out-of-bounds PCL read.
  static const vector<string> required_fields = {"x", "y", "z", "intensity", "ring", "timestamp"};
  for (const auto &field_name : required_fields)
  {
    ...
    if (!found)
    {
      ...
      if (!printed)
      {
        printed = true;
        string available;
        for (const auto &f : msg->fields) available += f.name + " ";
        printf("[hesai_handler] Missing required field '%s'. Available fields: [%s]. "
               "Check your Hesai driver's point cloud format.\n",
               field_name.c_str(), available.c_str());
      }
      return;
    }
  }
